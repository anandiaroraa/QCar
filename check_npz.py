...
# path tracking (left)
axs[0].plot(ref_x, ref_y, 'r--', label='Reference')
...
axs[0].set_title(f'Path Tracking (radius={radius}m)\nRMSE={np.sqrt(np.mean(tracking_error**2)):.3f}m  Mean={tracking_error.mean():.3f}m  Std={tracking_error.std():.3f}m  Max={tracking_error.max():.3f}m')
axs[0].legend()
axs[0].axis('equal')
axs[0].grid(True)
...
print(f"Std:  {std_error:.4f}  path tracking (left)")
axs[0].plot(ref_x, ref_y, 'r--', label='Reference')
axs[0].plot(wp_x, wp_y, 'y*', markersize=6, label='Reference waypoints')
axs[0].plot(x, y, 'b-', label='Actual')
axs[0].plot(x[0], y[0], 'go', markersize=10, label='Start')
