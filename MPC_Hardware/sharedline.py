...
def run_car(test_case, at_pushing_pose=True, path_tracking_config=None):
    ...
    line_length = 1.0
    n_points = 50
    cx = [data.car1.x + line_length * i / (n_points - 1) for i in range(n_points)]
    cy = [data.car1.y] * n_points
    cyaw = [math.pi / 2.0] * n_points
    ck = [0.0] * n_points
    print(f"First waypoint: ({cx[0]:.3f}, {cy[0]:.3f})")
    print(f"Last waypoint:  ({cx[-1]:.3f}, {cy[-1]:.3f})")
    ...
    while not rospy.is_shutdown() and (time.time() - start_time) < max_time:
        ...
        dist_traveled = math.hypot(state.x - cx[0], state.y - cy[0])
...
if __name__ == "__main__":
    ...
    for test_case in test_cases:
        results_dir = f'hardware_results_test_straight1{test_case}'
