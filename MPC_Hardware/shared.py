...
class Data():
    ...
    def update_car1_pose(self, msg):
        ...
        # ADD THIS:
        print(f"OptiTrack -> x:{pose.x:.4f} y:{pose.y:.4f} z:{pose.z:.4f} | roll:{np.deg2rad(rpy[0]):.4f} pitch:{np.deg2rad(rpy[1]):.4f} yaw:{np.deg2rad(rpy[2]):.4f} (rad) | roll:{np.rad2deg(rpy[0]):.2f} pitch:{np.rad2deg(rpy[1]):.2f} yaw:{np.rad2deg(rpy[2]):.2f} (deg)")
        
        self.car1.theta = rpy[2]
        self.car1.x = pose.x
        self.car1.y = pose.y
        self.car1.received = True
    """
...
def run_car(test_case, at_pushing_pose=True, path_tracking_config=None):
    ...
    data = Data()
    get_car1_pose = rospy.Subscriber("/natnet_ros/RigidBody/pose", PoseStamped, data.update_car1_pose)
    rospy.sleep(1)
    
    give_command1 = rospy.Publisher("/qcar/mux/ackermann_cmd_mux/input/navigation", AckermannDriveStamped, queue_size=10)
