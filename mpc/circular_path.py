...
def calc_circle_course(radius=2.0, ds=0.1, center_x=0.0, center_y=0.0, clockwise=False):
    """
    Circular path generator (drop-in alternative to calc_spline_course).

    Returns
    -------
    rx, ry, ryaw, rk, s
        Same format as cubic_spline_planner.calc_spline_course()
    """
    if radius <= 0:
    ...
    # Wrap yaw to [-pi, pi) (nice for MPC)
...
def demo_circle():
    """Plot circle path + yaw + curvature (like the PythonRobotics demo)."""
